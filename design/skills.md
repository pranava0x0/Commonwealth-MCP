...
1. **Establish the frame.** Resolve jurisdiction(s) and the entity in question first, using `registry.resolve_jurisdiction` and entity tools; on ambiguity, stop and surface candidates (never guess; the Fairfax trap in design/jurisdiction-resolution.md § 2.2).
...
3. **Escalation table.** Findings → next workflow, from design spec § 18. Escalations follow `next_actions` hints when present but are stated in the skill so they work on envelope data alone.
