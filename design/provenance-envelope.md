...
- **The publisher's own cadence is the yardstick**, exactly as this line already said. A source declaring `unknown` — 22 of them — is never judged: there is no promise to be behind, and inventing a threshold would be this project deciding what "current" means for someone else's data. `freshness_unavailable` already covers a source that reports no vintage at all, and the two are mutually exclusive by construction.
