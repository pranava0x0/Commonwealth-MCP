...
- V1 geo sources, as registered (corrected 2026-08-28; the original line predated the 0005 override and the revised § 6 seed set): VGIN statewide parcels, VGIN administrative boundaries, Fairfax County, Richmond City, and Charles City County, all via `arcgis`; the Towns of Leesburg (parcels and zoning) and Vienna (zoning only) followed on 2026-09-07 and closed the forcing set (design/source-registry.md § 6). Loudoun County itself is still unregistered. Routing is the top-two rule above — there is no authority table and no locality-first default (architecture.md decision 0005, Chosen).
