...
- V1 geo sources, as registered (corrected 2026-08-28; the original line predated the 0005 override and the revised § 6 seed set): VGIN statewide parcels, VGIN administrative boundaries, Fairfax County, Richmond City, and Charles City County, all via `arcgis`; the Towns of Leesburg (parcels and zoning) and Vienna (zoning only) followed on 2026-09-07 and closed the forcing set (design/source-registry.md § 6). Loudoun County (parcels and zoning) followed on 2026-09-10, which moved a Leesburg parcel's two answers from town-and-state to town-and-county: two `primary` publishers now cover that ground. The county's hospitals-and-urgent-care layer opened a health capability the same day (`health_facility.lookup`, `geo.find_health_facilities`, toolset `spatial`). Routing is the top-two rule above — there is no authority table and no locality-first default (architecture.md decision 0005, Chosen).
...
- Meetings: Phase 1 covers the localities whose agenda platforms have registered manifests (legistar adapter lands here); coverage honesty does the rest. **Shipped 2026-09-09** as `civic.search_meetings` over five Legistar clients (Richmond, Alexandria, Albemarle, Hampton, Petersburg), one adapter with the client identifier in the manifest. The platform publishes no cancellation field, so a cancellation is read from the publisher's comment and returned as a labelled reading; see `adapters/agenda_platform.py`.
